package com.nesscurie.utils.view;
...
public class SwitchButton extends CompoundButton {
    ...
    public SwitchButton(Context context, AttributeSet attrs, int defStyle) {
        ...
        mThumbDrawable = a.getDrawable(R.styleable.SwitchButton_thumbDrawable);
        mThumbColor = a.getColorStateList(R.styleable.SwitchButton_thumbColor);
        ...
        float margin = a.getDimension(R.styleable.SwitchButton_thumbMargin, density * DEFAULT_THUMB_MARGIN_DP);
        float marginLeft = a.getDimension(R.styleable.SwitchButton_thumbMarginLeft, margin);
        float marginRight = a.getDimension(R.styleable.SwitchButton_thumbMarginRight, margin);
        float marginTop = a.getDimension(R.styleable.SwitchButton_thumbMarginTop, margin);
        float marginBottom = a.getDimension(R.styleable.SwitchButton_thumbMarginBottom, margin);
        mThumbMargin.set(marginLeft, marginTop, marginRight, marginBottom);

        float thumbWidth = a.getDimension(R.styleable.SwitchButton_thumbWidth, 0);
        float thumbHeight = a.getDimension(R.styleable.SwitchButton_thumbHeight, 0);
        mThumbWidth = ceil(thumbWidth);
        mThumbHeight = ceil(thumbHeight);

        mThumbRadius = a.getDimension(R.styleable.SwitchButton_thumbRadius, -1);
        mBackRadius = a.getDimension(R.styleable.SwitchButton_backRadius, -1);

        mBackDrawable = a.getDrawable(R.styleable.SwitchButton_backDrawable);
        mBackColor = a.getColorStateList(R.styleable.SwitchButton_backColor);
        ...
        if (!mIsBackUseDrawable && mBackColor == null) {
            ...
            mNextBackColor = mBackColor.getColorForState(CHECKED_PRESSED_STATE, 0);
        }

        float thumbRangeRatio = a.getFloat(R.styleable.SwitchButton_thumbRangeRatio, DEFAULT_THUMB_RANGE_RATIO);
        mThumbRangeRatio = mThumbMargin.width() >= 0 ? Math.max(thumbRangeRatio, 1) : thumbRangeRatio;

        mAnimationDuration = a.getInteger(R.styleable.SwitchButton_animationDuration, DEFAULT_ANIMATION_DURATION);
        mProgressAnimator.setDuration(mAnimationDuration);

        mFadeBack = a.getBoolean(R.styleable.SwitchButton_fadeBack, true);
        mTintColor = a.getColor(R.styleable.SwitchButton_tintColor, DEFAULT_TINT_COLOR);
        mTextOn = a.getString(R.styleable.SwitchButton_textOn);
        mTextOff = a.getString(R.styleable.SwitchButton_textOff);
        mTextThumbInset = a.getDimensionPixelSize(R.styleable.SwitchButton_textThumbInset, 0);
        mTextExtra = a.getDimensionPixelSize(R.styleable.SwitchButton_textExtra, 0);
        mTextAdjust = a.getDimensionPixelSize(R.styleable.SwitchButton_textAdjust, 0);
        a.recycle();

        a = context.obtainStyledAttributes(attrs, new int[]{android.R.attr.focusable,
                android.R.attr.clickable});
        boolean focusable = a.getBoolean(0, true);
        @SuppressLint("ResourceType")
        boolean clickable = a.getBoolean(1, focusable);
    ...
    private Layout makeLayout(CharSequence text) {
        return new StaticLayout(text, mTextPaint, (int) Math.ceil(Layout
                .getDesiredWidth(text, mTextPaint)), Layout.Alignment.ALIGN_CENTER, 1.f, 0, false);
    }
     ...
     * textExtraSpace = textWidth + textExtra - (moveRange - thumbWidth
     * + max(thumbMargin.left, thumbMargin.right) + textThumbInset)
    ...
    private int measureWidth(int widthMeasureSpec) {
        ...
        if (widthMode == MeasureSpec.EXACTLY) {
            ...
            if (mThumbWidth != 0) {
                ...
                textExtraSpace = textWidth + mTextExtra - (moveRange - mThumbWidth
            ...
            moveRange = ceil(mThumbWidth * mThumbRangeRatio);
            textExtraSpace = ceil(textWidth + mTextExtra - (moveRange - mThumbWidth
                    + Math.max(mThumbMargin.left, mThumbMargin.right) + mTextThumbInset));
            ...
            contentSize = ceil(moveRange + Math.max(0, mThumbMargin.left)
                    + Math.max(0, mThumbMargin.right) + Math.max(0, textExtraSpace));
    ...
    private void setup() {
        ...
        if (mBackDrawable != null) {
            mBackDrawable.setBounds((int) mBackRectF.left, (int) mBackRectF.top,
                    ceil(mBackRectF.right), ceil(mBackRectF.bottom));
        }

        if (mOnLayout != null) {
            float onLeft = mBackRectF.left + (mBackRectF.width() + mTextThumbInset - mThumbWidth
                    - mThumbMargin.right - mOnLayout.getWidth()) / 2f - mTextAdjust;
        ...
        if (mOffLayout != null) {
            float offLeft = mBackRectF.right - (mBackRectF.width() + mTextThumbInset - mThumbWidth
                    - mThumbMargin.left - mOffLayout.getWidth()) / 2f - mOffLayout.getWidth() + mTextAdjust;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (switchText != null && textRectF != null) {
            ...
            colorAlpha = colorAlpha * alpha / 255;
            switchText.getPaint().setARGB(colorAlpha, Color.red(textColor),
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (Math.abs(deltaX) < mTouchSlop && Math.abs(deltaY) < mTouchSlop && time < mClickTimeout) {
                    ...
                    if (nextStatus != isChecked()) {
                        ...
                        setCheckedWithAnimator(nextStatus);
                    } else {
    ...
    public void setCheckedNoAnimator(boolean checked) {
        if (isChecked() != checked) {
            mProgressAnimator.setDuration(0);
            if (checked) {
                mProgressAnimator.setFloatValues(mProgress, 1f);
            } else {
                mProgressAnimator.setFloatValues(mProgress, 0);
    ...
    /**
     * @deprecated 去掉了setChecked中的动画, 直接使用 {@link #setChecked(boolean)}即可
     */
    @Deprecated
    public void setDefaultCheckedNoEvent(boolean checked) {
        if (mChildOnCheckedChangeListener == null) {
            setCheckedNoAnimator(checked);
        } else {
    ...
    private void setCheckedWithAnimator(final boolean checked) {
    ...
    /**
     * @deprecated 不需要使用这个方法, 在setOnCheckedChangeListener中使用buttonView.isPressed判断
     * eg: if (!buttonView.isPressed) return@setOnCheckedChangeListener
     */
    @Deprecated
    ...
    @Override
    public void toggle() {
        setCheckedWithAnimator(!isChecked());
    }
